...
<template>
  <div>
    <h1 class="text-title mb-2em font-bold text-center">Projects</h1>
    <article
      v-for="(series, index) in projectList"
      :key="index"
      slide-enter
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-1em">
        <a
          v-for="project in series.content"
          :key="project.path"
          :title="project.name"
          :href="project.path"
        ...
        >
          <div class="hover w-full">
            <div class="text-lg">
              {{ project.name }}
              <span
                v-if="project.path.startsWith('http')"
                class="text-sm op-50 i-icon-park-outline-efferent-three"
              />
            </div>
